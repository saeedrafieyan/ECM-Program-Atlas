...
DEFAULT_GTEX_FULL_EXPRESSION = Path(
    "data/raw/gtex_v11_sample_level/GTEx_Analysis_2025-08-22_v11_RNASeQCv2.4.3_gene_tpm.parquet"
)

DEFAULT_OUTPUT_DIR = Path("results/revision_classification_controls_fast")
...
class RepresentationJob:
    ...
    family: str
    kind: str
    expression_matrix_name: str
    programs: Sequence[ProgramGeneSet] | None = None
...
def save_figure(fig: go.Figure, name: str, html_dir: Path, png_dir: Path, width: int = 1450, height: int = 850) -> None:
...
def load_matrix(path: Path, dataset_name: str) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing {dataset_name} matrix:\n{path}")
...
def align_matrix_and_metadata(matrix: pd.DataFrame, metadata: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return matrix.loc[common].copy(), metadata.set_index("sample_id").loc[common].reset_index()
...
def sample_random_gene_sets(
    ...
    for i, size in enumerate(sizes, start=1):
        if size > len(universe):
            raise ValueError(f"Requested gene-set size {size} exceeds universe size {len(universe)}.")

        genes = rng.choice(universe, size=size, replace=False)
        programs.append(ProgramGeneSet(name=f"{prefix}_{i}", genes=tuple(sorted(genes.tolist()))))

    return programs


def zscore_train_test(train: pd.DataFrame, test: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def score_gene_sets_foldwise(
    ...
    for program in programs:
        ...
        if not genes:
            train_scores[program.name] = 0.0
            test_scores[program.name] = 0.0
        else:
            train_scores[program.name] = train_z[genes].mean(axis=1)
            test_scores[program.name] = test_z[genes].mean(axis=1)

    return train_scores.values, test_scores.values


def pca_foldwise(train_matrix: pd.DataFrame, test_matrix: pd.DataFrame, n_components: int = 9) -> tuple[np.ndarray, np.ndarray]:
...
def load_full_expression_selected_genes(
    ...
    print(f"[INFO] Loading full GTEx expression for {len(selected)} selected non-ECM genes.")
    ...
    if not keep_sample_cols:
        raise ValueError("No requested sample IDs found in full GTEx expression matrix.")

    sub = expr[expr["gene_symbol"].isin(selected)][["gene_symbol"] + keep_sample_cols].copy()

    if sub.empty:
        raise ValueError("No selected non-ECM genes found in full GTEx expression.")
    ...
    return sample_gene.loc[sample_ids]


def filter_classes(metadata: pd.DataFrame, label_col: str, min_samples_per_class: int) -> pd.DataFrame:
...
def build_folds(
    ...
    n_splits: int,
) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray, list[tuple[np.ndarray, np.ndarray]], LabelEncoder]:
    meta = filter_classes(metadata, label_col=label_col, min_samples_per_class=min_samples_per_class).copy()
    ...
    return meta, y, groups, y_str, folds, le


def precompute_representation_features(
    job: RepresentationJob,
    label_col: str,
    meta: pd.DataFrame,
    y: np.ndarray,
    groups: np.ndarray,
    ...
    expression_matrix = matrices[job.expression_matrix_name]
    sample_ids = meta["sample_id"].tolist()
    X_expr = expression_matrix.loc[sample_ids].copy()

    fold_features: list[FoldFeatures] = []
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(folds, start=1):
        ...
        if job.kind == "gene_set_scores":
            if job.programs is None:
                raise ValueError(f"Representation {job.name} requires programs.")
            X_train, X_test = score_gene_sets_foldwise(train_matrix, test_matrix, job.programs)

        elif job.kind == "pca9":
            X_train, X_test = pca_foldwise(train_matrix, test_matrix, n_components=9)

        else:
            raise ValueError(f"Unsupported representation kind: {job.kind}")
        ...
        fold_features.append(
            FoldFeatures(
                representation=job.name,
                representation_family=job.family,
                representation_kind=job.kind,
                repeat=job.repeat,
                label_col=label_col,
                fold=fold_idx,
                X_train=X_train,
...
def train_torch_model(
    ...
    model.eval()
    with torch.no_grad():
        return model(X_test_t.to(device)).argmax(dim=1).cpu().numpy()
...
def fit_predict_model(
    ...
    if model_name == "dense_mlp":
        ...
        X_train = scaler.fit_transform(X_train_raw)
        X_test = scaler.transform(X_test_raw)
        model = DenseMLP(X_train.shape[1], n_classes, hidden_dim=128, dropout=0.15)
        return train_torch_model(model, X_train, y_train, X_test, n_classes, max_epochs, batch_size, patience)

    if model_name == "residual_mlp":
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train_raw)
        X_test = scaler.transform(X_test_raw)
        model = ResidualMLP(X_train.shape[1], n_classes, hidden_dim=128, n_blocks=3, dropout=0.15)
        return train_torch_model(model, X_train, y_train, X_test, n_classes, max_epochs, batch_size, patience)
    ...
    if model_name == "ft_transformer_lite":
        ...
        X_train = scaler.fit_transform(X_train_raw)
        X_test = scaler.transform(X_test_raw)
        model = SimpleFTTransformer(X_train.shape[1], n_classes, d_token=64, n_heads=4, n_layers=3, dropout=0.15)
        return train_torch_model(model, X_train, y_train, X_test, n_classes, max_epochs, batch_size, patience)
    ...
    if model_name == "tabpfn":
        if n_classes > 10:
            raise RuntimeError("TabPFN skipped because the number of classes exceeds 10.")
    ...
    for model_name in model_names:
        ...
        try:
            y_pred = fit_predict_model(
                ...
                X_train_raw=features.X_train,
                y_train=features.y_train,
                X_test_raw=features.X_test,
                n_classes=features.n_classes,
                max_epochs=max_epochs,
                patience=patience,
                batch_size=batch_size,
            )
            metrics = evaluate(features.y_test, y_pred)
            status = "ok"
            error = ""

        except Exception as exc:
            print(f"    [FAILED] {model_name}: {exc}")
            y_pred = np.zeros_like(features.y_test)
            metrics = {
                "accuracy": np.nan,
        ...
        fold_records.append(
            {
                ...
                "representation_family": features.representation_family,
                "representation_kind": features.representation_kind,
                "model": model_name,
                "label_col": features.label_col,
                "repeat": features.repeat,
                "fold": features.fold,
                "status": status,
                "error": error,
                "n_train": features.X_train.shape[0],
                "n_test": features.X_test.shape[0],
                "n_train_subjects": features.n_train_subjects,
                "n_test_subjects": features.n_test_subjects,
                "n_classes": features.n_classes,
        ...
        if status == "ok":
            pred_meta = features.test_meta.copy()
            pred_meta["representation"] = features.representation
            pred_meta["representation_family"] = features.representation_family
            pred_meta["model"] = model_name
            pred_meta["label_col"] = features.label_col
            pred_meta["repeat"] = features.repeat
            pred_meta["fold"] = features.fold
            # Store encoded labels only to avoid massive inverse-transform bookkeeping here.
            pred_meta["y_true_encoded"] = features.y_test
            pred_meta["y_pred_encoded"] = y_pred
...
def summarize_random_families(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (family, model, label_col), group in summary_df.groupby(["representation_family", "model", "label_col"]):
        record = {
        ...
        for metric_name, col in metric_map.items():
            ...
            record[f"{metric_name}_family_mean"] = values.mean() if not values.empty else np.nan
            record[f"{metric_name}_family_std"] = values.std(ddof=0) if not values.empty else np.nan
            record[f"{metric_name}_family_min"] = values.min() if not values.empty else np.nan
            record[f"{metric_name}_family_max"] = values.max() if not values.empty else np.nan
        ...
        for model in sorted(summary_df["model"].unique()):
            ...
            fig = go.Figure(
                data=go.Bar(
                    ...
                    orientation="h",
                    error_x=dict(type="data", array=sub["balanced_accuracy_std"], visible=True),
                    customdata=sub[
                        ["representation_family", "macro_f1_mean", "accuracy_mean", "mean_n_features"]
                    ],
                    hovertemplate=(
                        "Representation: %{y}<br>"
            ...
            fig.update_layout(
                title=f"R2 representation control benchmark<br><sup>Task: {label_col}, model: {model}</sup>",
            ...
            save_figure(fig, f"r2_representation_benchmark_{label_col}_{model}", html_dir, png_dir)
        ...
        for model in sorted(family_summary["model"].unique()):
            ...
            fig = go.Figure(
                data=go.Bar(
                    ...
                    orientation="h",
                    error_x=dict(type="data", array=sub["balanced_accuracy_family_std"], visible=True),
                    customdata=sub[
                        [
            ...
            fig.update_layout(
                title=f"R2 representation-family benchmark<br><sup>Task: {label_col}, model: {model}</sup>",
                xaxis_title="Balanced accuracy",
            ...
            save_figure(fig, f"r2_representation_family_benchmark_{label_col}_{model}", html_dir, png_dir)


def write_report(summary_df: pd.DataFrame, family_summary_df: pd.DataFrame, fold_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    lines.append("\n## Models compared\n")
    for model in sorted(fold_df["model"].dropna().unique().tolist()):
    ...
    if summary_df.empty:
        ...
        for (label_col, model), group in summary_df.groupby(["label_col", "model"]):
            ...
            lines.append(
                ...
                f"{best['representation']} ({best['representation_family']}), "
                f"balanced accuracy = {best['balanced_accuracy_mean']:.3f} ± {best['balanced_accuracy_std']:.3f}, "
...
def main() -> None:
    ...
    parser.add_argument("--skip-non-ecm", action="store_true")
    ...
    matrices = {"matrisome": matrisome_matrix}

    representation_jobs: list[RepresentationJob] = [
        RepresentationJob(
            name="curated_9_ecm_programs",
            family="curated_ecm",
            kind="gene_set_scores",
            expression_matrix_name="matrisome",
            programs=programs,
        ),
        RepresentationJob(
            name="matrisome_pca9",
            family="matrisome_pca",
            kind="pca9",
            expression_matrix_name="matrisome",
            programs=None,
        ),
    ...
    for repeat in range(args.n_random_repeats):
        random_programs = sample_random_gene_sets(
        ...
        representation_jobs.append(
            RepresentationJob(
                name=f"random_matrisome_programs_r{repeat + 1}",
                family="random_matrisome",
                kind="gene_set_scores",
                expression_matrix_name="matrisome",
    ...
    if not args.skip_non_ecm:
        ...
        non_ecm_jobs: list[RepresentationJob] = []
        ...
        for repeat in range(args.n_random_repeats):
            ...
            non_ecm_jobs.append(
                RepresentationJob(
                    name=f"random_non_ecm_programs_r{repeat + 1}",
                    family="random_non_ecm",
                    kind="gene_set_scores",
                    expression_matrix_name="non_ecm",
                    programs=random_programs,
                    repeat=repeat + 1,
                )
            )

        matrices["non_ecm"] = load_full_expression_selected_genes(
        ...
        representation_jobs.extend(non_ecm_jobs)

    all_fold_records = []
    all_prediction_records = []

    for label_col in ["tissue_system", "tissue"]:
        print(f"\n[TASK] {label_col}")

        meta, y, groups, _y_str, folds, _le = build_folds(
            metadata=metadata,
            label_col=label_col,
        ...
        for job in representation_jobs:
            print(f"  [REPRESENTATION] {job.name}")

            fold_feature_list = precompute_representation_features(
                job=job,
                label_col=label_col,
                meta=meta,
                y=y,
            ...
            for fold_features in fold_feature_list:
                fold_records, prediction_records = run_models_on_features(
    ...
    if args.write_predictions and all_prediction_records:
